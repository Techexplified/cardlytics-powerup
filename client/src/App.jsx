...
function CardDetailsView() {
  ...
  function exportPDF() {
    const statLabel = STAT_LABELS[statType] || "Cards";
    const scopeLabel = (mode === "list" || statType === "cardsInList")
      ? `List: ${listName}`
      : `Board: ${boardName}`;
    ...
    const rows = filtered.map((card) => {
      const members = card.idMembers?.length
        ? card.idMembers.map((id) => memberMap[id]?.fullName || id.slice(0, 6)).join(", ")
        : "—";
      const created = formatDate(cardCreatedDate(card.id).toISOString());
      const due = card.due ? formatDate(card.due) : "—";
      const modified = formatDate(card.dateLastActivity);
      const list = listMap[card.idList] || listName;
      const done = card.dueComplete ? "✓" : "";
      const labels = card.labels?.map((l) => l.color || "none").join(", ") || "—";
      return `<tr>
        <td>${card.name}</td>
        <td>${members}</td>
        <td>${labels}</td>
        ...
        <td>${created}</td>
        <td class="${card.due && new Date(card.due) < new Date() && !card.dueComplete ? "overdue" : ""}">${due}</td>
        <td>${modified}</td>
        <td><span class="list-tag">${list}</span></td>
      </tr>`;
    }).join("");
...
<head>
  <meta charset="utf-8"/>
  <title>Cardlytics — ${statLabel}</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; font-size: 11px; color: #1a1a1a; background: #fff; padding: 28px 32px; }
    .header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px; padding-bottom: 14px; border-bottom: 2px solid #e0e0e0; }
    .header-left h1 { font-size: 20px; font-weight: 700; color: #111; }
    .header-left .scope { font-size: 12px; color: #555; margin-top: 3px; }
    .header-right { text-align: right; color: #888; font-size: 11px; line-height: 1.6; }
    .badge { display: inline-block; background: #f0f4ff; color: #3b5bdb; border-radius: 4px; padding: 2px 8px; font-size: 10px; font-weight: 600; margin-bottom: 6px; }
    .summary { display: flex; gap: 12px; margin-bottom: 20px; }
    .summary-item { background: #f7f7f7; border-radius: 6px; padding: 10px 16px; flex: 1; text-align: center; }
    .summary-item .num { font-size: 22px; font-weight: 700; color: #111; }
    .summary-item .lbl { font-size: 10px; color: #777; margin-top: 2px; text-transform: uppercase; letter-spacing: 0.4px; }
    table { width: 100%; border-collapse: collapse; }
    thead tr { background: #f5f5f5; }
    th { padding: 8px 10px; text-align: left; font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; color: #555; border-bottom: 1px solid #ddd; white-space: nowrap; }
    td { padding: 7px 10px; border-bottom: 1px solid #ebebeb; vertical-align: middle; color: #222; }
    tr:last-child td { border-bottom: none; }
    tr:nth-child(even) { background: #fafafa; }
    .center { text-align: center; }
    .overdue { color: #d32f2f; font-weight: 600; }
    .list-tag { background: #e8f4fd; color: #1565c0; border-radius: 3px; padding: 1px 6px; font-size: 10px; white-space: nowrap; }
...
<body>
  <div class="header">
    <div class="header-left">
      <div class="badge">Cardlytics</div>
      <h1>${statLabel}</h1>
      <div class="scope">${scopeLabel}</div>
    </div>
    <div class="header-right">
      <div>Exported: ${now}</div>
      <div>${filtered.length} card${filtered.length !== 1 ? "s" : ""}</div>
    </div>
  </div>
  <table>
    <thead>
      <tr>
        <th>Name</th><th>Assigned</th><th>Labels</th><th>Done</th>
  ...
  <div class="footer">
    <span>Generated by Cardlytics Power-Up</span>
    <span>${boardName}</span>
  </div>
    ...
    <div className="cd-root">
      ...
      <div className="cd-right">
        ...
        >
          ...
          <button className="cd-action-btn" onClick={exportPDF}>⬇ Export PDF</button>
        </div>

...
// ─── MAIN APP ────────────────────────────────────────────────────────────────
// ─── CUSTOMIZE STANDALONE VIEW (opened as its own fullscreen modal) ──────────
function CustomizeStandaloneView() {
  const params = useRef(new URLSearchParams(window.location.search));
  const boardId = params.current.get("boardId");
  const urlMode = params.current.get("mode");
  const urlListId = params.current.get("listId");
  const initialStatType = params.current.get("statType") || null;
  ...
  const [customizeStat, setCustomizeStat] = useState(initialStatType);
  ...
  useEffect(() => {
    async function load() {
      ...
      const [fullBoardCards, allLists, memberId, labels, boardInfo] = await Promise.all([
        getBoardCards(key, tkn, boardId),
        getBoardLists(key, tkn, boardId),
        getMemberId(key, tkn),
        getBoardLabels(key, tkn, boardId),
        getBoard(key, tkn, boardId),
      ]);

      const cardlyticsListIds = allLists
        .filter((l) => l.name.toLowerCase() === "cardlytics")
        .map((l) => l.id);
      const filtered = fullBoardCards.filter(
        (c) => !isTrackerCard(c.name) && !cardlyticsListIds.includes(c.idList),
      );

      setAllBoardCards(filtered);
      setLists(allLists);
      ...
      if (memberId) {
        ...
        if (membersRes.ok) setBoardMembers(await membersRes.json());
      }

      const computed = computeStats(filtered, memberId);
      computed.cardsInList = urlMode === "list" ? filtered.length : 0;
      setStats(computed);
  ...
  function computeFilteredCount(statType, filters) {
    ...
    if (hasExplicitFilters) {
      return applyFilters(allBoardCards, filters, currentMemberId).length;
    }
  ...
  async function handleSave(statType, cfg) {
    try {
      const key = TRELLO_API_KEY;
      const tkn = getStoredToken();
      if (!tkn || !boardId) return;

      let targetListId;
      if (urlMode === "list" && urlListId) {
        targetListId = urlListId;
      } else {
      ...
      const defaults = DEFAULT_STAT_CONFIG[statType];
      const filterConfig = {
        due: cfg.due || [],
        members: cfg.members || [],
        labels: cfg.labels || [],
      ...
      const count = hasActiveFilters
        ? applyFilters(allBoardCards, filterConfig, currentMemberId).length
        : (() => {
            const statFn = buildStatFilterMap(currentMemberId, null)[statType];
            return statFn && statType !== "cardsInList" && statType !== "all"
              ? allBoardCards.filter(statFn).length
              : allBoardCards.length;
          })();
      ...
      const metaTag =
        urlMode === "list" && urlListId
          ? `\n\n[_]: cardlytics:mode:list:listId:${urlListId}:statType:${statType}${filterStr}`
          : `\n\n[_]: cardlytics:mode:board:statType:${statType}${filterStr}`;

      const cardName = cfg.cardName || defaults.name;
      const desc = `${count} card(s) tracked by Cardlytics.${metaTag}`;
      const cover = cfg.cover || defaults.cover;

      const coverImageDataUrl = await generateStatCoverImage(count, cover, cfg.coverImage || null);
      const newCard = await createCard(key, tkn, targetListId, cardName, desc, cover, coverImageDataUrl);

      if (cfg.coverImage && trelloT) {
        try {
          localStorage.setItem(`cardlytics:customBg:${newCard.id}`, cfg.coverImage);
      ...
      const newView = {
        ...
        boardId,
        mode: urlMode === "list" && urlListId ? "list" : "board",
        listId: urlMode === "list" ? urlListId : null,
      };
      localStorage.setItem(
        `cardlytics:personalized:${boardId}`,
        JSON.stringify([...boardPersonalized, newView]),
      );

      if (trelloT) setTimeout(() => trelloT.closeModal(), 500);
    } catch (err) {
      console.error("Customize standalone save error:", err);
    }
  }
...
export default function App() {
  ...
  if (view === "customize") return <CustomizeStandaloneView />;
          ...
          <div className="header-actions">
           ...
           <button
              ...
              onClick={() => {
                ...
                trelloT.board("id").then((board) => {
                  const scopeParam =
                    mode === "list" && listId ? `&mode=list&listId=${listId}` : "";
                  trelloT.modal({
                    title: "Cardlytics",
                    url: `./index.html?view=customize&boardId=${board.id}${scopeParam}`,
                    fullscreen: true,
                  });
                });
              }}

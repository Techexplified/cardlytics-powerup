...
const PRIMARY_FILTER = {
  assigned: "assigned", dueThisWeek: "due", overdue: "due",
  unassigned: "assigned", withLabel: "labels", stale: "activity",
  createdToday: "activity", cardsInList: "list",
};
...
function deriveSmartName(statType, due, members, labels, lists, memberName, boardLabels, listData, membersData) {
  ...
  if (members.length === 1) {
    ...
    const displayName = memberObj
      ...
      : members[0] === "me" && memberName ? memberName.split(" ")[0] : null;
  ...
  useEffect(() => {
    ...
    function measure() {
      ...
      if (openUpward) {
        ...
        setCoords({ top: rect.top - clampedHeight - 4, left: rect.left, width: rect.width, maxHeight: clampedHeight, openUpward: true });
      } else {
        const clampedHeight = Math.min(maxHeight, Math.max(spaceBelow, 80));
        setCoords({ top: rect.bottom + 4, left: rect.left, width: rect.width, maxHeight: clampedHeight, openUpward: false });
      }
    }
    measure();
    if (!open) return;
  ...
  return createPortal(
    <div
      ...
      style={{
        ...
        zIndex: 2147483647,
        boxShadow: coords.openUpward ? "0 -4px 20px rgba(0,0,0,0.6)" : "0 4px 20px rgba(0,0,0,0.6)",
    ...
    <div ref={containerRef} style={{ flex: 1 }}>
      <div
        ...
        style={{
          ...
          background: "#1e1e1e", border: `1px solid ${open ? "#555" : "#3a3a3a"}`,
        ...
        {selected.length === 0 ? (
          ...
          selected.map((v) => (
            <span key={v} style={{
              ...
              padding: "1px 6px", fontSize: 11, color: "#d0d0d0",
            }}>
              {chipLabel ? chipLabel(v) : v}
              <span onClick={(e) => { e.stopPropagation(); toggle(v); }} style={{ color: "#555", cursor: "pointer", fontSize: 10 }}>✕</span>
            </span>
          ))
        )}
        <span style={{ marginLeft: "auto", color: "#444", fontSize: 10 }}>{open ? "▲" : "▼"}</span>
      </div>
        ...
        <div>
          {options.map((opt) => (
            <div
              ...
              style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", cursor: "pointer", fontSize: 12, color: "#bbb", background: "transparent", transition: "background 0.12s" }}
          ...
          {footer && (<><div style={{ borderTop: "1px solid #2a2a2a" }} />{footer}</>)}
        </div>
      </PortalDropdown>
    </div>
  );
  ...
  return (
    <div style={{ position: "absolute", bottom: 7, right: 8, display: "flex", zIndex: 1 }}>
      {visible.map((id, idx) => {
        ...
        const initials = m ? m.fullName.split(" ").map((w) => w[0]).join("").toUpperCase().slice(0, 2) : id.slice(0, 2).toUpperCase();
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          padding: "13px 16px", borderBottom: "1px solid #333",
        }}>
        ...
        <p style={{ fontSize: 12, color: "#666", padding: "10px 16px 6px", margin: 0 }}>Select a stat to configure</p>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
          <span style={{ fontSize: 10, fontWeight: 700, color: "#666", letterSpacing: "0.08em", textTransform: "uppercase" }}>Gradients</span>
          {!isPremium && (
            <span style={{
...
function SectionLabel({ children }) {
  return (
    <div style={{ fontSize: 10, fontWeight: 700, color: "#666", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 6 }}>
      {children}
    </div>
  );
}
...
function FilterRow({ label, icon, children }) {
  return (
    <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, width: 76, flexShrink: 0, fontSize: 12, color: "#777", paddingTop: 7 }}>
...
function ActiveFiltersSummary({ due, members, labels, lists, dueChipLabel, memberChipLabel, labelChipLabel, listChipLabel }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 9, fontWeight: 700, color: "#666", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 2 }}>
        Active filters
      </div>
      {groups.map((g) => (
...
function CardConfigModal({
  ...
  const liveCount = computeFilteredCount
    ? computeFilteredCount(statType, { due: selectedDue, members: selectedMembers, labels: selectedLabels, lists: selectedLists, customDateFrom, customDateTo })
    : statValue ?? 0;
  ...
  const memberOptions = (members && members.length > 0
    ? members
    : memberName ? [{ id: "me", fullName: memberName, avatarColor: "#0052cc" }] : []
  ).map((m) => ({
    value: m.id,
    label: m.fullName,
    render: () => {
      ...
      return (
        <div style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", minWidth: 0 }}>
          <div style={{ width: 22, height: 22, borderRadius: "50%", background: m.avatarColor || "#0052cc", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8, fontWeight: 700, color: "#fff", flexShrink: 0 }}>{initials}</div>
          <span style={{ fontSize: 12, color: "#ccc", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{m.fullName}</span>
        </div>
      );
    },
  }));

  function dueChipLabel(v) { return DUE_OPTIONS.find((o) => o.value === v)?.label || v; }
  ...
  function listChipLabel(id) { return (lists || []).find((l) => l.id === id)?.name || id; }
  ...
  const labelOptions = (boardLabels && boardLabels.length > 0 ? boardLabels : []).map((lbl) => {
    ...
    return {
      value: lbl.id, label: displayName,
      render: () => (
        <div style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", minWidth: 0 }}>
          <span style={{ width: 28, height: 14, borderRadius: 3, background: hex, display: "inline-block", flexShrink: 0 }} />
          <span style={{ color: displayName === lbl.color ? "#888" : "#ccc", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{displayName}</span>
        </div>
      ),
    };
  });
  ...
  function handleSaveFilters() {
    const snapshot = { due: selectedDue, members: selectedMembers, labels: selectedLabels, lists: selectedLists, customDateFrom, customDateTo };
    setSavedFilters(snapshot);
    setFiltersDirty(false);
    setJustSaved(true);
    if (!nameManuallyEdited) setCardName(smartName);
    setTimeout(() => setJustSaved(false), 2000);
  }

  function handleSave() {
    const filters = savedFilters || { due: selectedDue, members: selectedMembers, labels: selectedLabels, lists: selectedLists, customDateFrom, customDateTo };
    onSave(statType, { cardName: previewName, cover: coverColor, coverImage, ...filters });
  }

  const DueFilter = (
    <FilterRow label="Due" icon="🕐" key="due">
      <MultiSelect options={DUE_OPTIONS} selected={selectedDue} onChange={setDue} placeholder="any" chipLabel={dueChipLabel} footer={dueDateFooter} />
    </FilterRow>
  );
  const AssignedFilter = (
    <FilterRow label="Assigned" icon="👤" key="assigned">
      <MultiSelect options={memberOptions} selected={selectedMembers} onChange={setMembers} placeholder="any member" chipLabel={memberChipLabel} />
    </FilterRow>
  );
  const LabelsFilter = (
    <FilterRow label="Labels" icon="🏷" key="labels">
      <MultiSelect options={labelOptions} selected={selectedLabels} onChange={setLabels} placeholder="any label" chipLabel={labelChipLabel} />
    </FilterRow>
  );
  const ListFilter = (
    <FilterRow label="List" icon="☰" key="list">
      <MultiSelect options={listOptions} selected={selectedLists} onChange={setListsSel} placeholder="any list" chipLabel={listChipLabel} />
    </FilterRow>
  );
  const BoardFilter = (
    <FilterRow label="Board" icon="⊞" key="board">
      <select style={{
        ...
        color: "#ccc", fontSize: 12, padding: "6px 8px",
        fontFamily: "inherit", outline: "none", maxWidth: 160, cursor: "pointer",
      }}>
  ...
  const orderedFilters = [primary ? allFilters[primary] : null, BoardFilter, ...secondaryKeys.map((k) => allFilters[k])].filter(Boolean);

  const hasActiveFilters = selectedDue.length > 0 || selectedMembers.length > 0 || selectedLabels.length > 0 || selectedLists.length > 0;
  ...
  return (
    <div style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        {/* Header */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "13px 16px", borderBottom: "1px solid #333", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <button onClick={onBack} style={{ background: "none", border: "none", color: "#888", fontSize: 20, cursor: "pointer", padding: "0 4px", fontFamily: "inherit" }}>‹</button>
            <span style={{ fontSize: 14, fontWeight: 600, color: "#e0e0e0" }}>Dashcards — Track</span>
          </div>
          <button onClick={onClose} style={{ background: "none", border: "none", color: "#666", fontSize: 16, cursor: "pointer", padding: "2px 6px", borderRadius: 4 }}>✕</button>
        </div>

        {/* Body */}
        <div style={{ display: "flex", overflow: "hidden", flex: 1, minHeight: 0 }}>
          {/* Left: preview */}
          <div style={{ width: 200, flexShrink: 0, padding: 14, borderRight: "1px solid #2e2e2e", display: "flex", flexDirection: "column", gap: 8, overflowY: "auto" }}>
            <div style={{ background: "#1a1a1a", border: "1px solid #333", borderRadius: 8, overflow: "hidden" }}>
              ...
              <div style={{ padding: "8px 10px" }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: "#ccc", lineHeight: 1.35, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                  {previewName}
                </div>
              </div>
            </div>
            ...
            <ActiveFiltersSummary
              due={selectedDue} members={selectedMembers} labels={selectedLabels} lists={selectedLists}
              dueChipLabel={dueChipLabel} memberChipLabel={memberChipLabel} labelChipLabel={labelChipLabel} listChipLabel={listChipLabel}
            />

            {savedFilters && !filtersDirty && (
              <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 10px", background: "#1a2a1a", border: "1px solid #2a4a2a", borderRadius: 8 }}>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      cursor: "pointer", color: isActive ? "#5a9fff" : "#777",
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <>
                  <div>
                    ...
                    <input
                      ...
                      style={{ width: "100%", background: "#1e1e1e", border: "1px solid #3a3a3a", borderRadius: 6, padding: "7px 10px", color: "#e0e0e0", fontSize: 12, fontFamily: "'DM Sans', sans-serif", outline: "none", boxSizing: "border-box", transition: "border-color 0.15s" }}
                    ...
                    {!nameManuallyEdited && hasActiveFilters && (
                      <div style={{ fontSize: 10, color: "#555", marginTop: 4 }}>
                        ...
                        <span style={{ color: "#29b6f6", cursor: "pointer" }} onClick={() => { setCardName(smartName); setNameManuallyEdited(true); }}>edit</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                      <div>
                        <div style={{ fontSize: 11, color: "#666", marginBottom: 4 }}>Board</div>
                        <select style={{ width: "100%", background: "#1e1e1e", border: "1px solid #3a3a3a", borderRadius: 6, color: "#ccc", fontSize: 12, padding: "7px 10px", fontFamily: "inherit", outline: "none", cursor: "pointer" }}>
                          <option>This board</option>
                          <option>All boards</option>
                        </select>
                      </div>
                      <div>
                        <div style={{ fontSize: 11, color: "#666", marginBottom: 4 }}>Member</div>
                        <select style={{ width: "100%", background: "#1e1e1e", border: "1px solid #3a3a3a", borderRadius: 6, color: "#ccc", fontSize: 12, padding: "7px 10px", fontFamily: "inherit", outline: "none", cursor: "pointer" }}>
                          <option>Me (SB)</option>
                  ...
                  <div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                      ...
                      <button style={{ fontSize: 12, color: "#5a9fff", background: "none", border: "none", cursor: "pointer", fontFamily: "inherit", padding: 0 }}>+ Add filter</button>
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      {orderedFilters}
                    </div>
                  ...
                  {hasActiveFilters && (
                    <div style={{ display: "flex", alignItems: "center", gap: 10, paddingTop: 4 }}>
                      <button
                        ...
                        style={{
                          background: justSaved ? "#1a3a1a" : filtersDirty ? "#0052cc" : "#1e2e1e",
                          ...
                          cursor: filtersDirty ? "pointer" : "default", transition: "all 0.2s",
                          display: "flex", alignItems: "center", gap: 6,
                        }}
                      >
                        {justSaved ? "✓ Saved!" : filtersDirty ? "Save filters" : "✓ Filters saved"}
                      </button>
                      {filtersDirty && <span style={{ fontSize: 11, color: "#666" }}>Save to update the card count</span>}
                    </div>
                  )}

                  <div style={{ display: "flex", justifyContent: "center", paddingTop: 4 }}>
                    <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#1e1e1e", border: "1px solid #333", display: "flex", alignItems: "center", justifyContent: "center", color: "#555", fontSize: 14 }}>↓</div>
                  </div>
                </>
              )}
              ...
              {activeTab === "alerts" && (
                <>
                  <p style={{ fontSize: 13, color: "#888", margin: "0 0 4px" }}>Get notified when this stat hits a threshold.</p>

                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "#1e1e1e", border: "1px solid #2e2e2e", borderRadius: 8, padding: "11px 14px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <span style={{ fontSize: 16 }}>🔔</span>
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: "#d0d0d0" }}>Count exceeds 5</div>
                        <div style={{ fontSize: 11, color: "#666", marginTop: 2 }}>Notify via Trello notification</div>
                      </div>
                    </div>
                    <button
                      onClick={() => setAlertOn((v) => !v)}
                      style={{ width: 38, height: 20, borderRadius: 10, border: "none", cursor: "pointer", position: "relative", background: alertOn ? "#0052cc" : "#333", transition: "background 0.2s", flexShrink: 0 }}
                    >
                      <div style={{ position: "absolute", top: 3, left: alertOn ? 21 : 3, width: 14, height: 14, borderRadius: "50%", background: "#fff", transition: "left 0.2s", boxShadow: "0 1px 3px rgba(0,0,0,0.4)" }} />
                    </button>
                  </div>

                  <button style={{ width: "100%", background: "none", border: "1px dashed #2e2e2e", borderRadius: 8, padding: "10px", color: "#666", fontSize: 13, fontFamily: "inherit", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                    <span>+</span><span>Add alert</span>
                  </button>

                  <div style={{ background: "#1e1800", border: "1px solid #3d3000", borderRadius: 8, padding: "10px 14px", fontSize: 12, color: "#b89a40", display: "flex", alignItems: "flex-start", gap: 8, lineHeight: 1.5 }}>
              ...
              {activeTab === "style" && (
                <>
                  ...
                  <div>
                    <SectionLabel>
                      Cover image{" "}
                      <span style={{ color: "#555", fontWeight: 400, textTransform: "none", letterSpacing: 0 }}>(optional — overrides color)</span>
                    </SectionLabel>
        ...
        {/* Footer */}
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, padding: "12px 16px", borderTop: "1px solid #2e2e2e", flexShrink: 0 }}>
          <button onClick={onClose} style={{ background: "none", border: "1px solid #3a3a3a", borderRadius: 6, padding: "7px 18px", color: "#aaa", fontSize: 13, fontFamily: "inherit", cursor: "pointer" }}>Cancel</button>
          <button
            onClick={handleSave}
            style={{ background: "#0052cc", border: "none", borderRadius: 6, padding: "7px 18px", color: "#fff", fontSize: 13, fontWeight: 600, fontFamily: "'DM Sans', sans-serif", cursor: "pointer" }}
